...
export default function CharacterProfile({ scope, values, inherited = {}, onChange, onPromote, extras = {}, onlySections }: {
  ...
  extras?: Record<string, React.ReactNode>; onlySections?: string[];
}) {
  const dev = useUi().mode === "developer";
  const [editing, setEditing] = useState<string | null>(null);
  const [text, setText] = useState("");
  return <div className="character-profile">
    <p className="scope-note">{scope === "global" ? "角色库定义 · 这个人长期是谁" : "仅本故事 · 这个人在当前故事中的状态"}</p>
    {CHARACTER_SECTIONS.filter(([title]) => !onlySections || onlySections.includes(title)).map(([title, fields]) => <section key={title} className="card">
      <h3>{title}</h3>
      {fields.filter(([key]) => scope === "global" || key !== "bio").map(([key, label]) => {
        ...
        return <div className="character-understanding" key={key}>
          <div className="row"><b className="grow">{label}</b><span className="soft-tag">{scope === "global" ? "角色库默认" : fromGlobal ? "继承角色库" : "仅本故事"}</span></div>
          ...
          {editing === key ? <div><textarea aria-label={label} value={text} onChange={e => setText(e.target.value)} />
            ...
            : <div className="toolbar"><button className="small" onClick={() => { setEditing(key); setText(value); }}>{value ? "修改" : "补充"}</button>
              ...
              {scope === "scenario" && inherited[key] && !fromGlobal && <button className="small" onClick={() => onChange({ [key]: inherited[key] })}>继承角色库</button>}</div>}
    ...
    {onPromote && <button onClick={onPromote}>保存为角色库新版本</button>}
  </div>;
}
