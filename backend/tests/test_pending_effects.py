...
def test_effects_respect_packet_boundary_and_interstitial_excludes_hidden_context(monkeypatch):
    from app.domain.schemas import Branch, BranchSource, BranchStatus
    from app.providers.base import TextResponse
    engine, state, route, noop = _engine_and_state()
    secret = "设施负责人提前伪造了完整的撤离记录"
    state.scenario_snapshot["drama"]["truth_model"] = "hidden：" + secret
    state.world.truth["hidden"] = False
    monkeypatch.setattr(engine, "_persist", noop)
    monkeypatch.setattr(engine, "_push", noop)
    branch = Branch(id="private", session_id=state.id, arc_id=state.current_arc().id,
                    source=BranchSource.FREE, label="检查走廊",
                    base_versions=engine._branch_base_versions(state))
    state.branches.append(branch)
    async def director(*a, **kw):
        return {"outcome": {"title": "观察", "text": "你观察走廊。",
                           "effects": [secret, "你走近门边。"]}, "directive": {}}, route
    monkeypatch.setattr(engine, "_director_output", director)
    async def text(role, **kw):
        serialized=json.dumps(kw,ensure_ascii=False)
        assert secret not in serialized
        assert "truth_model" not in serialized
        assert "known_state" in serialized
        return None,route,TextResponse(content=json.dumps({"lines":["灯光微微闪烁。"]}))
    engine.router=SimpleNamespace(call_text=text)
    async def run():
        await engine._plan_branch(state,branch)
        assert branch.packet is not None
        assert branch.outcome.effects==["你走近门边。"]
        branch.status=BranchStatus.GENERATING
        await engine._interstitial_effects(state.id,branch.id)
        assert "灯光微微闪烁。" in branch.outcome.effects
    asyncio.run(run())


def test_terminal_pending_branch_does_not_keep_waiting_text_visible():
    from app.domain.schemas import Branch, BranchSource, BranchStatus, OutcomeSpec
    engine,state,_,_=_engine_and_state()
    b=Branch(id="terminal",session_id=state.id,arc_id=state.current_arc().id,
             source=BranchSource.FREE,status=BranchStatus.FAILED,
             outcome=OutcomeSpec(title="x",text="x",effects=["旧的等待文字"]),
             base_versions=engine._branch_base_versions(state))
    state.branches.append(b);state.pending_freeform_id=b.id
    state.player.status="FAILED_RECOVERABLE"
    for status in [BranchStatus.FAILED,BranchStatus.CANONICAL,BranchStatus.CANCELLED]:
        b.status=status
        view=engine.player_view(state)
        assert view["pending_effects"]==[]
        assert view["pending_phase"]==""
        assert view["pending_label"]==""


def test_completed_video_never_waits_for_optional_interstitial(monkeypatch):
    from app.domain.schemas import Branch, BranchSource, BranchStatus
    engine,state,_,noop=_engine_and_state()
    b=Branch(id="fast_video",session_id=state.id,arc_id=state.current_arc().id,
             source=BranchSource.FREE,status=BranchStatus.PREDICTED,
             base_versions=engine._branch_base_versions(state))
    state.branches.append(b)
    for name in ["_persist","_push","_phase_sleep","_plan_branch","_narrate_branch",
                 "_shoot_branch","_assemble_branch","_maybe_publish"]:
    ...
    async def run():
        ...
        async def video(*args):await started.wait()
        monkeypatch.setattr(engine,"_interstitial_effects",optional)
        monkeypatch.setattr(engine,"_generate_branch_media",video)
        await asyncio.wait_for(engine._pipeline_inner(state.id,b.id,False),timeout=1)
        await asyncio.wait_for(cancelled.wait(),timeout=1)
        assert b.status==BranchStatus.READY
    asyncio.run(run())
