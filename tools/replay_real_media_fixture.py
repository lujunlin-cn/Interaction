...
async def main():
    ...
    s=SessionState.model_validate(raw);s.id=os.getenv('FIXTURE_SESSION','acceptance_media_replay')
    ...
    s.world.version=recs[0].base_versions.world
    initial=RuntimeEngine(None)._bootstrap(s.scenario_version_id,s.scenario_id,s.scenario_snapshot)
    s.drama=initial.drama;s.drama.revision=recs[0].base_versions.drama
    ...
    (OUT/(s.id+'_origin.json')).write_text(json.dumps({'fixture_session':s.id,'origin_session':origin,'purpose':'real artifact replay plus HTTP transport fault injection; not new Provider generation','branch_ids':[b.id for b in recs],'initial_position':1,'decision_open_at':s.player.decision_open_at,'fingerprint':recs[0].fingerprint,'expiry_extended_for_paused_inspection':True},ensure_ascii=False,indent=2))
    print(s.id)
asyncio.run(main())
