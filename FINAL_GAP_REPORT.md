...
- Current FULL E2E is **PARTIAL**, because standard-view/outfit Relay edits and the full Player journey remain incomplete. The latest Relay continuation successfully produced 4K candidate assets for the UI-created Claire/Victor records; eight edit attempts returned transient 502/network failures, with no Fal image fallback. The Relay credential is configured only in ignored `backend/.env`; no credential is committed. Two supplied Fal keys are configured and enabled but were not probed in this continuation. Exact historic image HTTP count remains unknown. See `BIOHAZARD_FULL_E2E_ACCEPTANCE.md` and `docs/acceptance/biohazard_full_e2e/RELAY_VISUAL_CONTINUATION.md`.
